...
/usr/bin/time pip install Cython >> $EDGYR_LOGS/pytorch-1.7.log 2>&1

# see https://github.com/numpy/numpy/issues/18131
/usr/bin/time pip install \
  "numpy<=1.19.4" \
  pillow >> $EDGYR_LOGS/pytorch-1.7.log 2>&1
...
cd torchvision
/usr/bin/time python setup.py install >> $EDGYR_LOGS/pytorch-1.7.log 2>&1
cd ..
...
/usr/bin/time python $EDGYR_HOME/Installers/etc/test-pytorch.py >> $EDGYR_LOGS/pytorch-1.7.log 2>&1
