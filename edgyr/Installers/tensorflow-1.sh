...
/usr/bin/time sudo apt-get install -qqy --no-install-recommends \
  ...
  liblapack-dev \
  zip \
  zlib1g-dev >> $HOME/logs/tensorflow-1.log 2>&1
...
/usr/bin/time pip install --extra-index-url \
  https://developer.download.nvidia.com/compute/redist/jp/v44 'tensorflow<2.0'  >> $HOME/logs/tensorflow-1.log 2>&1
pip list --format=columns >> $HOME/logs/tensorflow-1.log 2>&1

echo "Installing R keras package"
/usr/bin/time Rscript -e "source('~/Installers/R/keras.R')" >> $HOME/logs/tensorflow-1.log 2>&1
